...
//        User user = userMapper.selectOne(wrapper);
    ...
    @ResponseBody
    public void download(@RequestParam("bucketName") String bucketName, @RequestParam("files") List<String> files) throws InterruptedException {
//        User user = (User)session.getAttribute("user");
        QueryWrapper wrapper = new QueryWrapper();
        wrapper.eq("username","sunqq");
        User user = userMapper.selectOne(wrapper);
        ...
        files.forEach(file->{
            pool.submit(new Runnable() {
                ...
                public void run() {
                    ...
                    URL url = s3.generatePresignedUrl(bucketName,file, calendar.getTime());
...
//                   config.withResumeOnRetry(true);
                    PresignedUrlDownload download = manager.download(request, new File("/Users/cooper/Desktop/S3Test/"+file), config);
